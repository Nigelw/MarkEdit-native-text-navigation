---
name: release
description: Cut a new release of the MarkEdit Native Text Navigation extension: bump the version, update the changelog, tag, push, and publish a GitHub release with the user script attached for the in-app updater.
---

# Release MarkEdit Native Text Navigation

This extension has an in-app self-updater in `markedit-native-text-navigation.js`. Installed copies poll `api.github.com/repos/Nigelw/MarkEdit-native-text-navigation/releases/latest`, compare the release tag against the script's `CURRENT_VERSION`, and when a newer version exists, download the release's `markedit-native-text-navigation.js` asset and overwrite their own installed script file with it.

A release is usable by the updater only if all of these agree:

1. `package.json` `version` is the new version.
2. The `CURRENT_VERSION` constant and header comment in `markedit-native-text-navigation.js` are the new version.
3. The GitHub release for `v<version>` has a `markedit-native-text-navigation.js` asset that is exactly that updated script.

There is no build step in this repo. The root `markedit-native-text-navigation.js` file is the distributed asset.
...
2. Update `markedit-native-text-navigation.js` so both the `// Version:` header and `CURRENT_VERSION` constant match the new version.
...
4. Verify the script carries the new version:
   `grep -c "<new-version>" markedit-native-text-navigation.js`
   The count should be at least 2 because both the header and `CURRENT_VERSION` should have changed.

5. Commit the release files:
   `git add package.json markedit-native-text-navigation.js CHANGELOG.md`
...
8. Publish the GitHub release with the script attached as the updater asset:
   `gh release create v<version> --title "v<version>" --notes "<changelog section>" markedit-native-text-navigation.js`

9. Verify the latest release exposes the asset the updater downloads:
   ```sh
   url=$(curl -sS "https://api.github.com/repos/Nigelw/MarkEdit-native-text-navigation/releases/latest" \
     | node -e "let d='';process.stdin.on('data',c=>d+=c).on('end',()=>{const a=JSON.parse(d).assets||[];const m=a.find(x=>x.name==='markedit-native-text-navigation.js');console.log(m?m.browser_download_url:'MISSING')})")
   echo "asset url: $url"
